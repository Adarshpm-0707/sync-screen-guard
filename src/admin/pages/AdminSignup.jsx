...
export default function AdminSignup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!isPlaceholder) {
        // Register user with is_admin: true in user_metadata
        await supabase.auth.signUp({
          ...
          // Ignore rate limit or network errors since local backup is stored
        });
      }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center px-4 font-sans relative select-none">
      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl relative z-10">
        ...
        <div className="flex flex-col items-center text-center space-y-4 mb-8">
          ...
          <div>
            <h1 className="font-display text-xl font-black tracking-wider text-white uppercase">Console Registration</h1>
            <p className="text-[10px] text-slate-450 font-bold uppercase tracking-widest mt-1">Create admin personnel privileges</p>
          </div>
        </div>
        ...
        {success && (
          <div className="flex items-start space-x-2.5 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-xs font-semibold text-emerald-400 mb-6">
            ...
            <span>Success! Admin user created. Redirecting to login...</span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5 text-left">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="newadmin@syncarmor.in"
              />
            </div>
          </div>
          ...
          <div>
            <label className="block text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mb-2">Confirm Password</label>
            <div className="relative">
              <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-slate-500 pointer-events-none">
                <Lock className="h-4 w-4" />
              </span>
